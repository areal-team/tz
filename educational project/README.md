# Технологии для реализации учебного проекта
- Draw.io
- Git / Github
- Vue.js (3.5) / Quasar / Vite
- Node.JS (20) / NestJS (10). Пакеты, которые рекомендуется использовать в работе:
...
## Репозиторий
Создать Git репозиторий https://github.com/<ваш логин>/areal-hr-test-2024. Коммититься нужно как можно чаще, при проверке будет учитываться содержательность сообщений коммита и порции, которыми коммиты осуществляются.

## Структура проекта

**.git** - *Хранятся структурные папки репозитория*  
**/api/** - *Backend приложение на Node.js*  
**/node_modules/** - *Хранение библиотек сторонних производителей и зависимости, файл должен быть исключен из репозитория*  
  **/migrations/** - *Миграции для базы данных*  
**/app/** - *Frontend приложение на Vue.js*  
  **/node_modules/** - *Хранение библиотек сторонних производителей и зависимости, файл должен быть исключен из репозитория*  
**/containers** - *Инструкции по созданию и использованию Docker образов*  
  **/api/**  
     **/Dockerfile** - *Инструкция по созданию Docker образа для Backend*  
  **/app/**  
     **/Dockerfile** - *Инструкция по созданию Docker образа для Frontend*  
**/docs/** - *Документация по приложению в формате md, для сборки используется проект VitePress*  
**.gitignore** - *Файл в системе контроля версий Git для определения игнорируемых файлов и каталогов*  
**.env** - *Используемые переменные окружения для приложений, файл должен быть исключен из репозитория*  
**.env.example** - *Пример переменных окружения для приложений*  
**docker-compose.yml** - *Файл Docker Compose, который будет содержать инструкции, необходимые для запуска и настройки сервисов*  
**README.md** - *Краткая документация по запуску приложению*  
## Задание
...
Для доступа к веб-приложению пользователю требуется ввести его логин и пароль. В веб-приложении должен вестись список пользователей и их доступы. Поля у пользователей:
  - Фамилия
  - Имя
  - Отчество
  - Логин
...
<table>
    <tr>
        ...
        <th>Ожидаемая готовность задания</th>
    </tr>
    <tr>
        <td>44</td>
